...
def key_presenter_over(back_bytes: bytes, greenscreen_url: str, audio_url: str = "") -> dict:
    ...
    try:
        with _video_slot("editor"):
            ...
            audio = ""
            if audio_url:
                audio = _os.path.join(tmp, "audio.mp4")
                with open(audio, "wb") as f:
                    f.write(_req.get(audio_url, timeout=180).content)

            # VIDEO-AS-TRUTH (omniflash grammar): the person keeps their original
            # full-frame framing and we edit only what surrounds them. Key the green
            # and composite the presenter over the generated background plate at 1:1.
            fc = (
